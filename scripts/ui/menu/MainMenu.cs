...
public partial class MainMenu : Control
{
    ...
    public override void _Ready()
    {
        ...
        {
            SoundManager.PlayJukebox();
            SoundManager.JukeboxPaused = !settings.AutoplayJukebox.Value;
        }
        else
        {
    ...
    public override void _Process(double delta)
    {
        ...
        if (SoundManager.Song.Stream != null)
        {
            ...
            SoundManager.Song.VolumeDb = Mathf.Lerp(
                ...
                Phoenyx.Quitting ? -80 : -80 + 70 * (float)Math.Pow(settings.VolumeMusic.Value / 100, 0.1) * (float)Math.Pow(settings.VolumeMaster.Value / 100, 0.1),
                (float)Math.Clamp(delta * 2, 0, 1));
        }
